...
exports.get_my_order_details = async (req, res) => {
    try {
        ...
        if (!order) {
            return res.status(403).json({ message: "Bu sipariş size ait değil veya bulunamadı!" });
        }
...
exports.get_my_orders = async (req, res) => {
    try {
        ...
        if (!orders || orders.length === 0) {
            return res.status(404).json({ message: "Henüz siparişiniz bulunmamaktadır!" });
        }

        res.json({ orders });
    } catch (error) {
        console.error("Siparişleri getirirken hata oluştu:", error);
        res.status(500).json({ message: "Siparişler getirilirken bir hata oluştu!", error });
    }
}
...
exports.get_order_details = async (req, res) => {
    try {
        ...
        if (orderProducts.length === 0) {
            return res.status(404).json({ message: "Bu sipariş için ürün bulunamadı." });
        }
        ...
        res.status(500).json({ message: "Bir hata oluştu, lütfen tekrar deneyin.", error });
    }
}
...
exports.complete_order_after_payment = async (orderId) => {
    try {
        console.log("complete_order_after_payment fonksiyonu çalıştı, orderId:", orderId);
        const order = await Order.findByPk(orderId);
